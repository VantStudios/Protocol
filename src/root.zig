pub const PROTOCOL = 2168;
pub const MINECRAFT_VERSION = "1.26.40";
pub const SHIELD_NETWORK_ID = 387;

pub const NBT = @import("nbt");

pub const Data = @import("./data/root.zig");
pub const AbilityIndex = @import("./enums/ability-index.zig").AbilityIndex;
pub const AbilityLayerType = @import("./enums/ability-layer-type.zig").AbilityLayerType;
pub const ActorDataId = @import("./enums/actor-data-id.zig").ActorDataId;
pub const ActorDataType = @import("./enums/actor-data-type.zig").ActorDataType;
pub const ActorFlags = @import("./enums/actor-flags.zig").ActorFlags;
pub const AttributeName = @import("./enums/attribute-name.zig").AttributeName;
pub const ClientPrediction = @import("./enums/client-prediction.zig").ClientPrediction;
pub const CompressionMethod = @import("./enums/compression-method.zig").CompressionMethod;
pub const ContainerId = @import("./enums/container-id.zig").ContainerId;
pub const ContainerName = @import("./enums/container-name.zig").ContainerName;
pub const ContainerType = @import("./enums/container-type.zig").ContainerType;
pub const CreativeItemCategory = @import("./enums/creative-item-category.zig").CreativeItemCategory;
pub const Difficulty = @import("./enums/difficulty.zig").Difficulty;
pub const DimensionType = @import("./enums/dimension-type.zig").DimensionType;
pub const DisconnectReason = @import("./enums/disconnect-reason.zig").DisconnectReason;
pub const GameMode = @import("./enums/gamemode.zig").Gamemode;
pub const InputData = @import("./enums/input-data.zig").InputData;
pub const InputMode = @import("./enums/input-mode.zig").InputMode;
pub const InteractAction = @import("./enums/interact-action.zig").InteractAction;
pub const InteractionMode = @import("./enums/interaction-mode.zig").InteractionMode;
pub const InventoryActionSource = @import("./enums/inventory-action-source.zig").InventoryActionSource;
pub const ItemStackResponseStatus = @import("./enums/item-stack-response-status.zig").ItemStackResponseStatus;
pub const Packet = @import("./enums/packet.zig").Packet;
pub const PermissionLevel = @import("./enums/permission-level.zig").PermissionLevel;
pub const PlayMode = @import("./enums/play-mode.zig").PlayMode;
pub const PlayStatusEnum = @import("./enums/play-status.zig").PlayStatus;
pub const PlayerActionType = @import("./enums/player-action-type.zig").PlayerActionType;
pub const PlayerListAction = @import("./enums/player-list-action.zig").PlayerListAction;
pub const ReleaseItemAction = @import("./enums/release-item-action.zig").ReleaseItemAction;
pub const ResourcePackResponse = @import("./enums/resource-pack-response.zig").ResourcePackResponse;
pub const SoundEvent = @import("./enums/sound-event.zig").SoundEvent;
pub const StackRequestActionType = @import("./enums/stack-request-action-type.zig").StackRequestActionType;
pub const TextType = @import("./enums/text-type.zig").TextType;
pub const TransactionType = @import("./enums/transaction-type.zig").TransactionType;
pub const TriggerType = @import("./enums/trigger-type.zig").TriggerType;
pub const UseItemAction = @import("./enums/use-item-action.zig").UseItemAction;
pub const UseItemOnEntityAction = @import("./enums/use-item-on-entity-action.zig").UseItemOnEntityAction;
pub const Login = @import("./login/root.zig");
pub const ActorEventPacket = @import("./packets/actor-event.zig").ActorEventPacket;
pub const ActorEventType = @import("./packets/actor-event.zig").ActorEventType;
pub const AddEntityPacket = @import("./packets/add-entity.zig").AddEntityPacket;
pub const AddItemActorPacket = @import("./packets/add-item-actor.zig").AddItemActorPacket;
pub const AddPlayerPacket = @import("./packets/add-player.zig").AddPlayerPacket;
pub const AnimatePacket = @import("./packets/animate.zig").AnimatePacket;
pub const AnimateAction = @import("./packets/animate.zig").AnimateAction;
pub const SwingSource = @import("./packets/animate.zig").SwingSource;
pub const AnvilDamagePacket = @import("./packets/anvil-damage.zig").AnvilDamagePacket;
pub const AvailableActorIdentifiersPacket = @import("./packets/available-actor-identifiers.zig").AvailableActorIdentifiersPacket;
pub const AvailableCommandsPacket = @import("./packets/available-commands.zig").AvailableCommandsPacket;
pub const PacketCommandEnum = @import("./packets/available-commands.zig").PacketCommandEnum;
pub const PacketCommand = @import("./packets/available-commands.zig").PacketCommand;
pub const PacketOverload = @import("./packets/available-commands.zig").PacketOverload;
pub const PacketParameter = @import("./packets/available-commands.zig").PacketParameter;
pub const PacketDynamicEnum = @import("./packets/available-commands.zig").PacketDynamicEnum;
pub const BlockActorDataPacket = @import("./packets/block-actor-data.zig").BlockActorDataPacket;
pub const BlockEventPacket = @import("./packets/block-event.zig").BlockEventPacket;
pub const BlockEventType = @import("./packets/block-event.zig").BlockEventType;
pub const BlockPickRequestPacket = @import("./packets/block-pick-request.zig").BlockPickRequestPacket;
pub const ChangeDimensionPacket = @import("./packets/change-dimension.zig").ChangeDimensionPacket;
pub const ChunkRadiusUpdatePacket = @import("./packets/chunk-radius-update.zig").ChunkRadiusUpdate;
pub const ClientCacheStatusPacket = @import("./packets/client-cache-status.zig").ClientCacheStatus;
pub const ClientToServerHandshake = @import("./packets/client-to-server-handshake.zig").ClientToServerHandshake;
pub const ClientboundUpdateSoundDataPacket = @import("./packets/clientbound-update-sound-data.zig").ClientboundUpdateSoundDataPacket;
pub const CommandOutputPacket = @import("./packets/command-output.zig").CommandOutputPacket;
pub const CommandOutputMessage = @import("./packets/command-output.zig").CommandOutputMessage;
pub const CommandRequestPacket = @import("./packets/command-request.zig").CommandRequestPacket;
pub const CommandOriginType = @import("./packets/command-request.zig").CommandOriginType;
pub const ContainerClosePacket = @import("./packets/container-close.zig").ContainerClosePacket;
pub const ContainerOpenPacket = @import("./packets/container-open.zig").ContainerOpenPacket;
pub const CreativeContentPacket = @import("./packets/creative-content.zig").CreativeContentPacket;
pub const DimensionDataPacket = @import("./packets/dimension-data.zig").DimensionDataPacket;
pub const DimensionDefinition = @import("./packets/dimension-data.zig").DimensionDefinition;
pub const DisconnectPacket = @import("./packets/disconnect.zig").Disconnect;
pub const DisconnectMessages = @import("./packets/disconnect.zig").DisconnectMessages;
pub const InteractPacket = @import("./packets/interact.zig").InteractPacket;
pub const InventoryContentPacket = @import("./packets/inventory-content.zig").InventoryContentPacket;
pub const InventorySlotPacket = @import("./packets/inventory-slot.zig").InventorySlotPacket;
pub const InventoryTransactionPacket = @import("./packets/inventory-transaction.zig").InventoryTransactionPacket;
pub const ItemRegistryPacket = @import("./packets/item-registry.zig").ItemRegistryPacket;
pub const ItemRegistryEntry = @import("./packets/item-registry.zig").ItemRegistryEntry;
pub const ItemStackRequestPacket = @import("./packets/item-stack-request.zig").ItemStackRequestPacket;
pub const ItemStackResponsePacket = @import("./packets/item-stack-response.zig").ItemStackResponsePacket;
pub const LevelChunkPacket = @import("./packets/level-chunk.zig").LevelChunk;
pub const LevelEventPacket = @import("./packets/level-event.zig").LevelEventPacket;
pub const LevelEvent = @import("./packets/level-event.zig").LevelEvent;
pub const LevelSoundEventPacket = @import("./packets/level-sound-event.zig").LevelSoundEventPacket;
pub const LoginPacket = @import("./packets/login.zig").Login;
pub const MobArmorEquipmentPacket = @import("./packets/mob-armor-equipment.zig").MobArmorEquipmentPacket;
pub const MobEquipmentPacket = @import("./packets/mob-equipment.zig").MobEquipmentPacket;
pub const MoveActorDeltaPacket = @import("./packets/move-actor-delta.zig").MoveActorDeltaPacket;
pub const MovePlayerPacket = @import("./packets/move-player.zig").MovePlayerPacket;
pub const MoveMode = @import("./packets/move-player.zig").MoveMode;
pub const TeleportationCause = @import("./packets/move-player.zig").TeleportationCause;
pub const NetworkChunkPublisherUpdatePacket = @import("./packets/network-chunk-publisher-update.zig").NetworkChunkPublisherUpdate;
pub const NetworkSettingsPacket = @import("./packets/network-settings.zig").NetworkSettings;
pub const PacketViolationWarningPacket = @import("./packets/packet-violation-warning.zig").PacketViolationWarningPacket;
pub const PlaySoundPacket = @import("./packets/play-sound.zig").PlaySoundPacket;
pub const PlayStatusPacket = @import("./packets/play-status.zig").PlayStatus;
pub const PlayerActionPacket = @import("./packets/player-action.zig").PlayerActionPacket;
pub const PlayerAuthInputPacket = @import("./packets/player-auth-input.zig").PlayerAuthInputPacket;
pub const PlayerListPacket = @import("./packets/player-list.zig").PlayerListPacket;
pub const PlayerLocationPacket = @import("./packets/player-location.zig").PlayerLocationPacket;
pub const PlayerLocationType = @import("./packets/player-location.zig").PlayerLocationType;
pub const PlayerSkinPacket = @import("./packets/player-skin.zig").PlayerSkinPacket;
pub const PlayerUpdateEntityOverridesPacket = @import("./packets/player-update-entity-overrides.zig").PlayerUpdateEntityOverridesPacket;
pub const PlayerUpdateEntityOverridesUpdateType = @import("./packets/player-update-entity-overrides.zig").UpdateType;
pub const RemoveEntityPacket = @import("./packets/remove-entity.zig").RemoveEntityPacket;
pub const RequestChunkRadiusPacket = @import("./packets/request-chunk-radius.zig").RequestChunkRadius;
pub const RequestNetworkSettingsPacket = @import("./packets/request-network-settings.zig").RequestNetworkSettings;
pub const ResourcePackResponsePacket = @import("./packets/resource-pack-response.zig").ResourcePackClientResponsePacket;
pub const ResourcePackStackPacket = @import("./packets/resource-pack-stack.zig").ResourcePackStackPacket;
pub const ResourcePacksInfoPacket = @import("./packets/resource-packs-info.zig").ResourcePacksInfoPacket;
pub const ServerToClientHandshake = @import("./packets/server-to-client-handshake.zig").ServerToClientHandshake;
pub const ServerboundDiagnosticsPacket = @import("./packets/serverbound-diagnostics.zig").ServerboundDiagnosticsPacket;
pub const SetActorDataPacket = @import("./packets/set-actor-data.zig").SetActorDataPacket;
pub const SetActorMotionPacket = @import("./packets/set-actor-motion.zig").SetActorMotionPacket;
pub const SetPlayerGameTypePacket = @import("./packets/set-player-game-type.zig").SetPlayerGameTypePacket;
pub const SetScorePacket = @import("./packets/set-score.zig").SetScorePacket;
pub const ScoreInfo = @import("./packets/set-score.zig").ScoreInfo;
pub const ScorerType = @import("./packets/set-score.zig").ScorerType;
pub const SetScoreboardIdentityPacket = @import("./packets/set-scoreboard-identity.zig").SetScoreboardIdentityPacket;
pub const SetScoreboardIdentityEntry = @import("./packets/set-scoreboard-identity.zig").Entry;
pub const SetScoreboardAction = @import("./packets/set-scoreboard-identity.zig").SetScoreboardAction;
pub const SetTitlePacket = @import("./packets/set-title.zig").SetTitlePacket;
pub const TitleType = @import("./packets/set-title.zig").TitleType;
pub const StartGamePacket = @import("./packets/start-game.zig").StartGamePacket;
pub const StructureBlockUpdatePacket = @import("./packets/structure-block-update.zig").StructureBlockUpdatePacket;
pub const StructureBlockType = @import("./packets/structure-block-update.zig").StructureBlockType;
pub const StructureEditorData = @import("./packets/structure-block-update.zig").StructureEditorData;
pub const StructureRedstoneSaveMode = @import("./packets/structure-block-update.zig").StructureRedstoneSaveMode;
pub const StructureSettings = @import("./packets/structure-block-update.zig").StructureSettings;
pub const SubChunkPacket = @import("./packets/sub-chunk.zig").SubChunkPacket;
pub const SubChunkData = @import("./packets/sub-chunk.zig").SubChunkData;
pub const TakeItemActorPacket = @import("./packets/take-item-actor.zig").TakeItemActorPacket;
pub const TextPacket = @import("./packets/text.zig").TextPacket;
pub const TransferPacket = @import("./packets/transfer.zig").TransferPacket;
pub const UpdateAbilitiesPacket = @import("./packets/update-abilities.zig").UpdateAbilitiesPacket;
pub const UpdateAttributesPacket = @import("./packets/update-attributes.zig").UpdateAttributesPacket;
pub const UpdateBlockPacket = @import("./packets/update-block.zig").UpdateBlockPacket;
pub const UpdateTradePacket = @import("./packets/update-trade.zig").UpdateTradePacket;
pub const VoxelShapesPacket = @import("./packets/voxel-shapes.zig").VoxelShapesPacket;
pub const AbilityLayer = @import("./types/ability-layer.zig").AbilityLayer;
pub const AbilitySet = @import("./types/ability-set.zig").AbilitySet;
pub const AttributeModifier = @import("./types/attribute-modifier.zig").AttributeModifier;
pub const Attribute = @import("./types/attribute.zig").Attribute;
pub const BlockPosition = @import("./types/block-position.zig").BlockPosition;
pub const Face = @import("./types/block-position.zig").Face;
pub const ChunkCoords = @import("./types/chunk-coords.zig").ChunkCoords;
pub const CreativeGroup = @import("./types/creative-group.zig").CreativeGroup;
pub const CreativeItem = @import("./types/creative-item.zig").CreativeItem;
pub const DataItem = @import("./types/data-item.zig").DataItem;
pub const Experiments = @import("./types/experiments.zig").Experiments;
pub const FullContainerName = @import("./types/full-container-name.zig").FullContainerName;
pub const GameRules = @import("./types/game-rules.zig");
pub const InventoryTransactionData = @import("./types/inventory-transaction-data.zig");
pub const ItemInstanceUserData = @import("./types/item-instance-user-data.zig").ItemInstanceUserData;
pub const ItemStackRequest = @import("./types/item-stack-request.zig").ItemStackRequest;
pub const ItemStackResponse = @import("./types/item-stack-response.zig").ItemStackResponse;
pub const ItemUseTransaction = @import("./types/item-use-transaction.zig").ItemUseTransaction;
pub const MoveDeltaFlags = @import("./types/move-delta-flags.zig").MoveDeltaFlags;
pub const NetworkBlockTypeDefinition = @import("./types/network-block-type-definition.zig").NetworkBlockTypeDefinition;
pub const NetworkItemInstanceDescriptor = @import("./types/network-item-instance-descriptor.zig").NetworkItemInstanceDescriptor;
pub const NetworkItemStackDescriptor = @import("./types/network-item-stack-descriptor.zig").NetworkItemStackDescriptor;
pub const PlayerBlockAction = @import("./types/player-block-action.zig").PlayerBlockAction;
pub const PlayerListEntry = @import("./types/player-list-entry.zig").PlayerListEntry;
pub const PropertySyncData = @import("./types/property-sync-data.zig").PropertySyncData;
pub const RequestedResourcePack = @import("./types/requested-resource-pack.zig").RequestedResourcePack;
pub const ResourceIdVersions = @import("./types/resource-id-versions.zig").ResourceIdVersions;
pub const ResourcePackDescriptor = @import("./types/resource-pack-descriptor.zig").ResourcePackDescriptor;
pub const ResourcePacksClientRequest = @import("./types/resource-packs-client-request.zig").ResourcePacksClientRequest;
pub const Rotation = @import("./types/rotation.zig").Rotation;
pub const SerializableCells = @import("./types/serializable-cells.zig").SerializableCells;
pub const SerializableVoxelShape = @import("./types/serializable-voxel-shape.zig").SerializableVoxelShape;
pub const SerializedSkin = @import("./types/serialized-skin.zig").SerializedSkin;
pub const ServerTelemetryData = @import("./types/server-telemetry-data.zig").ServerTelemetryData;
pub const StackResponseContainerInfo = @import("./types/stack-response-container-info.zig").StackResponseContainerInfo;
pub const StackResponseSlotInfo = @import("./types/stack-response-slot-info.zig").StackResponseSlotInfo;
pub const Uuid = @import("./types/uuid.zig").Uuid;
pub const Vector2f = @import("./types/vector2f.zig").Vector2f;
pub const Vector3f = @import("./types/vector3f.zig").Vector3f;
